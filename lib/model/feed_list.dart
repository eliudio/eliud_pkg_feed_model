...
typedef FeedWidgetProvider = Function(FeedModel? value);

class FeedListWidget extends StatefulWidget with HasFab {
  final AppModel app;
  final BackgroundModel? listBackground;
  final FeedWidgetProvider? widgetProvider;
  final bool? readOnly;
  final String? form;
  //final FeedListWidgetState? state;
  ...
  FeedListWidgetState createState() {
    return FeedListWidgetState();
  }

  @override
  Widget? fab(BuildContext context) {
    if ((readOnly != null) && readOnly!) return null;
    var state = FeedListWidgetState();
    return state.fab(
      context,
    );
  }
...
//            app: accessState.app,
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return FeedForm(app: widget.app, value: value, formAction: action);
    } else {
      return null;
    }
  }
