...
typedef ProfileWidgetProvider = Function(ProfileModel? value);

class ProfileListWidget extends StatefulWidget with HasFab {
  final AppModel app;
  final BackgroundModel? listBackground;
  final ProfileWidgetProvider? widgetProvider;
  final bool? readOnly;
  final String? form;
  //final ProfileListWidgetState? state;
  ...
  ProfileListWidgetState createState() {
    return ProfileListWidgetState();
  }

  @override
  Widget? fab(BuildContext context) {
    if ((readOnly != null) && readOnly!) return null;
    var state = ProfileListWidgetState();
    return state.fab(
      context,
    );
  }
...
//            app: accessState.app,
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return ProfileForm(app: widget.app, value: value, formAction: action);
    } else {
      return null;
    }
  }
