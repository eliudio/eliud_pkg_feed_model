...
typedef PostWidgetProvider = Function(PostModel? value);

class PostListWidget extends StatefulWidget with HasFab {
  final AppModel app;
  final BackgroundModel? listBackground;
  final PostWidgetProvider? widgetProvider;
  final bool? readOnly;
  final String? form;
  //final PostListWidgetState? state;
  ...
  PostListWidgetState createState() {
    return PostListWidgetState();
  }

  @override
  Widget? fab(BuildContext context) {
    if ((readOnly != null) && readOnly!) return null;
    var state = PostListWidgetState();
    return state.fab(
      context,
    );
  }
