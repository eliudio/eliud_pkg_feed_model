...
typedef FeedMenuChanged = Function(
  String? value,
  int? privilegeLevel,
);

/* 
 * FeedMenuDropdownButtonWidget is the drop down widget to allow to select an instance of FeedMenu
 */
class FeedMenuDropdownButtonWidget extends StatefulWidget {
  final AppModel app;
  final int? privilegeLevel;
  final String? value;
  final FeedMenuChanged? trigger;
  final bool? optional;

  /* 
   * construct a FeedMenuDropdownButtonWidget
   */
  FeedMenuDropdownButtonWidget(
      {required this.app,
      this.privilegeLevel,
      this.value,
      this.trigger,
  ...
  /* 
   * create state of FeedMenuDropdownButtonWidget
   */
  @override
  State<StatefulWidget> createState() {
    return _FeedMenuDropdownButtonWidgetState(value);
  }
}

class _FeedMenuDropdownButtonWidgetState
    extends State<FeedMenuDropdownButtonWidget> {
  FeedMenuListBloc? bloc;
  String? value;

  _FeedMenuDropdownButtonWidgetState(this.value);

  @override
  void didChangeDependencies() {
    bloc = BlocProvider.of<FeedMenuListBloc>(context);
  ...
  List<Widget> _widgets(FeedMenuModel value) {
    var app = widget.app;
    var widgets = <Widget>[];
...
//        final values = state.values;
            ...
            ((false) == true)
                ? Container(
                    ...
                    child: dropdownButton<String>(
                      ...
                      items: items,
                      value: value,
                      hint: text(widget.app, context, 'Select a feedMenu'),
            ...
            if (value != null)
              Align(
                  ...
                  child: button(
                    ...
                    label: 'Update',
                    onPressed: () {
                      updateComponent(context, widget.app, 'feedMenus', value,
                          (newValue, _) {
                        setState(() {
                          value = value;
                        });
  ...
  void _onPrivilegeLevelChange(int? value) {
    BlocProvider.of<FeedMenuListBloc>(context).add(FeedMenuChangeQuery(
